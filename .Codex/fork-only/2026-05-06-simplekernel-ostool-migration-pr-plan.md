...
`.Codex/fork-only/2026-05-14-ostool-architecture-refactor-plan.md` 中，并且应先于本文档中的后续 feature PR 执行。
...
| PR-03 | `feature/debug-artifact-pipeline` | OSTool | 中 | 基于已合入的 Cargo outcome / runtime artifact helper，先补窄 R2 artifact registry 和 object-tools 边界，再默认生成 `disassembly` 和 `elf_info` 文件；后续再按需要扩展 nm/bin 调试产物 | PR-02、R1d/R1e 兼容切片、R2 窄切片 | 对一个 bare-metal ELF 默认生成 `<elf-stem>.disassembly` 和 `<elf-stem>.elf_info`，路径稳定可被后续步骤引用；不依赖宿主机 binutils；不改变 runtime `.bin` 语义 |
...
graph TD
    ...
    R1["R1d/R1e compatible seams"] --> R2["R2 narrow artifact/object-tools boundary"]
