...
- R1d 已完成：`CargoBuildPipeline::execute()` 返回 `CargoBuildOutcome`，Cargo executable selection 位于
...
| 阶段 | 阶段文档 | 加载时机 |
|---|---|---|
| R0 | `.Codex/fork-only/2026-05-14-ostool-r0-contract-baseline.md` | 开始任何代码移动前；需要确认现有行为边界或 Docker 验证基线时 |
| R1 | `.Codex/fork-only/2026-05-14-ostool-r1-invocation-architecture-plan.md` | 开始移除 `Tool` 中心化架构前；需要确认目录结构、命名、任务拆分或 review 维度时 |
...
`.Codex/fork-only/2026-05-14-ostool-r0-contract-baseline.md`。
...
`.Codex/fork-only/2026-05-14-ostool-r1-invocation-architecture-plan.md`。
...
- 短期可以继续通过 `ctx::OutputArtifacts` 做兼容桥接，但新增 debug artifact state 不应复用
  runtime `.bin` 字段。
- 新增内部 object tools 边界，例如 `ostool/src/artifact/object_tools.rs`，只负责定位和调用 Rust
...
- 把 `build/mod.rs` 的 orchestration 整理为更明确的阶段：
...
| 重构项 | 解锁 feature |
|---|---|
| R1d/R1e 兼容切片、R2 窄 artifact/object-tools 边界 | F03 debug artifact pipeline |
