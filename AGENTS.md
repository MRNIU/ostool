...
- 代码文件顶部必须有文件级注释，说明该文件的主要功能和职责边界；Rust 源文件优先使用
  模块级文档注释（`//!`），脚本和前端文件按语言惯例使用文件头注释。
- 函数级别必须有相应注释，说明函数职责、关键输入输出、副作用或错误边界；Rust 公开 API
  使用文档注释（`///`），内部函数也应按复杂度补充简洁注释，避免只复述函数名的空洞注释。
