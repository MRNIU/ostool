...
The client `connect_serial_stream()` owns separate buffer endpoints for each direction. Remote closure or failure ends input after received bytes are consumed; stopping local output closes the WebSocket independently, without waiting for the reader to exit first.
